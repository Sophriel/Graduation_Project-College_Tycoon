...
public class PiUI : MonoBehaviour
{
    ...
    {
        menuPosition = screenPos - new Vector2(Screen.width / 2, Screen.height / 2);
        openedMenu = true;
        foreach (PiPiece pi in piList)
        {
        ...
        Vector2 tempPos = menuPosition / 2;
        switch (openTransition)
        {
            case TransitionType.Scale:
    ...
    public class PiData
    {
        ...
        public void OnInspectorGUI(SerializedProperty sprop, PiUI menu, System.Action AddSlice, System.Action<PiData> RemoveSlice, System.Action<int> angleUpdate)
        {
            ...
            if (icon == null)
                icon = Sprite.Create(angleTexture,new Rect(0,0,angleTextureSize,angleTextureSize),Vector2.zero, 1.0f,0,SpriteMeshType.FullRect,Vector4.zero,false);
